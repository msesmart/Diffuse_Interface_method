...
double surf_x[l+1][m+1],surf_y[l+1][m+1],curv[l+1][m+1];
...
{
    double temp1,temp2,angle,deltaAngle,angle_start,drx,dry,dr,R; int i,n;
    np=0; R=rad1; deltaAngle=asin(0.8*pdr/R);

    temp1=(xc1+rad1*2.0-xc2)*0.5/rad1; angle_start=acos(1.0-temp1);
    n=(int)((2.0*PI-2.0*angle_start)*R/pdr);
    deltaAngle=(2.0*PI-2.0*angle_start)/n;
    angle=angle_start;
    for(i=1;i<=n+1;i++)
    {
        np++;
        if(np>npar-1){printf("ERROR: np=%d exceed the npar",np);exit(0);}
        xp[np]=cos(angle)*rad1+xc1;
        yp[np]=sin(angle)*rad1+yc1;
        if(np>1)
        {
            //temp1=0.5*(1.0-cos(angle)); temp2=0.5*(1.0-cos(angle-deltaAngle));
            temp1=0.5; temp2=temp1;
            drx=xp[np]-xp[np-1]; dry=yp[np]-yp[np-1]; dr=sqrt(drx*drx+dry*dry);
            surf[np-1]=0.5*(temp1+temp2)*dr;
        }
        angle=angle+deltaAngle;
    }
    angle=angle_start-PI+deltaAngle;
    for(i=2;i<=n;i++)
    {
        np++;
        if(np>npar-1){printf("ERROR: np=%d exceed the npar",np);exit(0);}
        xp[np]=cos(angle)*rad2+xc2;
        yp[np]=sin(angle)*rad2+yc2;
        if(np>1)
        {
            //temp1=0.5*(1.0-cos(angle)); temp2=0.5*(1.0-cos(angle-deltaAngle));
            temp1=0.0; temp2=temp1;
            drx=xp[np]-xp[np-1]; dry=yp[np]-yp[np-1]; dr=sqrt(drx*drx+dry*dry);
            surf[np-1]=0.5*(temp1+temp2)*dr;
        }
        angle=angle+deltaAngle;
...
{
    int i,next,n=60; double temp1,temp2,ratio=0.4,f_nplus1,temp;
    cal_length2p();
...
//    output_particle(surf_new,"part_diffuse_new",0);
    while(n>0)
    {
        ...
        for(i=1;i<=np;i++)
        {
            ...
            f_nplus1=f_nplus1-0.5*temp; surf_new[i]=f_nplus1*length2p[i];
...
combine_aga:i=1;
    while(i<=np)
    {
        if(length2p[i]<=pdr_min){combine_part_deform(i); mark=1;}
        if(i==np)next=1; else next=i+1; if(next==np)nextTwo=1; else nextTwo=next+1;
        temp1=(xp[nextTwo]-xp[i])*(xp[next]-xp[i])+(yp[nextTwo]-yp[i])*(yp[next]-yp[i]);
        temp2=(xp[i]-xp[nextTwo])*(xp[next]-xp[nextTwo])+(yp[i]-yp[nextTwo])*(yp[next]-yp[nextTwo]);
        if(temp1<=0.0||temp2<=0.0){combine_part_deform(i); mark=1;}
        i++;
    }
...
void distribute_surfaceTension(void) //distribute surf_p[npar][2] to surf_x & surf_y
{
    int i_before,i,j,i_next,i_nextTwo,ieuler,jeuler,ieuler_next,jeuler_next,i_start,i_end,j_start,j_end,ii,jj;
    double xeuler,yeuler,temp1,temp2,temp3,temp4,x_pro,y_pro,deltaF;
    #pragma omp parallel for schedule(static) private(j) num_threads(TN)
    for(i=1;i<l;i++)
    for(j=1;j<m;j++)
    { surf_x[i][j]=0.0; surf_y[i][j]=0.0; }
    ...
    jeuler_next,i_start,i_end,j_start,j_end,ii,jj,xeuler,yeuler,temp1,temp2,temp3,temp4,x_pro,y_pro,deltaF) num_threads(TN)
    for(i=1;i<=np;i++)
    {
        if(i==np)i_next=1; else i_next=i+1; if(i==1)i_before=np; else i_before=i-1;
        if(i_next==np)i_nextTwo=1; else i_nextTwo=i_next+1;
        ...
        {
            if(fin[ii][jj]>=0.001&&fin[ii][jj]<=0.999)
            {
                temp1=xp[i_next]-xp[i]; temp2=yp[i_next]-yp[i];
                deltaF=3.0*1.414213562373095/epn*fin[ii][jj]*fin[ii][jj]*(1.0-fin[ii][jj])*(1.0-fin[ii][jj]);
                if(fabs(temp1)<=fabs(temp2))  //use y_pro
                {
                    ...
                    if(fabs(temp4)>=0.2*length2p[i])
                    {
                        ...
                        {
                            temp1=0.5*(yp[i_before]+yp[i]); temp2=0.5*(yp[i_next]+yp[i]);
                            surf_x[ii][jj]=surf_x[ii][jj]+deltaF*((y_pro-temp1)*NSTp[i][0]-(y_pro-temp2)*NSTp[i_before][0])/(temp2-temp1);
                            surf_y[ii][jj]=surf_y[ii][jj]+deltaF*((y_pro-temp1)*NSTp[i][1]-(y_pro-temp2)*NSTp[i_before][1])/(temp2-temp1);
                            surf_x[ii][jj]=surf_x[ii][jj]+deltaF*((y_pro-yp[i])*MSTp[i_next][0]-(y_pro-yp[i_next])*MSTp[i][0])/(yp[i_next]-yp[i]);
                            surf_y[ii][jj]=surf_y[ii][jj]+deltaF*((y_pro-yp[i])*MSTp[i_next][1]-(y_pro-yp[i_next])*MSTp[i][1])/(yp[i_next]-yp[i]);
                        }
                        else if(y_pro-temp4==0.0||(y_pro-temp4)*(y_pro-yp[i_next])<0.0)
                        {
                            temp1=0.5*(yp[i_next]+yp[i_nextTwo]); temp2=0.5*(yp[i_next]+yp[i]);
                            surf_x[ii][jj]=surf_x[ii][jj]+deltaF*((y_pro-temp1)*NSTp[i][0]-(y_pro-temp2)*NSTp[i_next][0])/(temp2-temp1);
                            surf_y[ii][jj]=surf_y[ii][jj]+deltaF*((y_pro-temp1)*NSTp[i][1]-(y_pro-temp2)*NSTp[i_next][1])/(temp2-temp1);
                            surf_x[ii][jj]=surf_x[ii][jj]+deltaF*((y_pro-yp[i])*MSTp[i_next][0]-(y_pro-yp[i_next])*MSTp[i][0])/(yp[i_next]-yp[i]);
                            surf_y[ii][jj]=surf_y[ii][jj]+deltaF*((y_pro-yp[i])*MSTp[i_next][1]-(y_pro-yp[i_next])*MSTp[i][1])/(yp[i_next]-yp[i]);
                        }
                        else ;
                ...
                else  //use x_pro
                {
                    ...
                    if(fabs(temp4)>=0.2*length2p[i])
                    {
                        ...
                        {
                            temp1=0.5*(xp[i_before]+xp[i]); temp2=0.5*(xp[i_next]+xp[i]);
                            surf_x[ii][jj]=surf_x[ii][jj]+deltaF*((x_pro-temp1)*NSTp[i][0]-(x_pro-temp2)*NSTp[i_before][0])/(temp2-temp1);
                            surf_y[ii][jj]=surf_y[ii][jj]+deltaF*((x_pro-temp1)*NSTp[i][1]-(x_pro-temp2)*NSTp[i_before][1])/(temp2-temp1);
                            surf_x[ii][jj]=surf_x[ii][jj]+deltaF*((x_pro-xp[i])*MSTp[i_next][0]-(x_pro-xp[i_next])*MSTp[i][0])/(xp[i_next]-xp[i]);
                            surf_y[ii][jj]=surf_y[ii][jj]+deltaF*((x_pro-xp[i])*MSTp[i_next][1]-(x_pro-xp[i_next])*MSTp[i][1])/(xp[i_next]-xp[i]);
                        }
                        else if(x_pro-temp4==0.0||(x_pro-temp4)*(x_pro-xp[i_next])<0.0)
                        {
                            temp1=0.5*(xp[i_next]+xp[i_nextTwo]); temp2=0.5*(xp[i_next]+xp[i]);
                            surf_x[ii][jj]=surf_x[ii][jj]+deltaF*((x_pro-temp1)*NSTp[i][0]-(x_pro-temp2)*NSTp[i_next][0])/(temp2-temp1);
                            surf_y[ii][jj]=surf_y[ii][jj]+deltaF*((x_pro-temp1)*NSTp[i][1]-(x_pro-temp2)*NSTp[i_next][1])/(temp2-temp1);
                            surf_x[ii][jj]=surf_x[ii][jj]+deltaF*((x_pro-xp[i])*MSTp[i_next][0]-(x_pro-xp[i_next])*MSTp[i][0])/(xp[i_next]-xp[i]);
                            surf_y[ii][jj]=surf_y[ii][jj]+deltaF*((x_pro-xp[i])*MSTp[i_next][1]-(x_pro-xp[i_next])*MSTp[i][1])/(xp[i_next]-xp[i]);
                        }
                        else ;
