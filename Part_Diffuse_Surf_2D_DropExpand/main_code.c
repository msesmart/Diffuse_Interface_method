...
void reinitiate_interface(int itime)
{
    int i,j;
	double x,y,temp,r,R;
	R=rad+itime*dt;
	#pragma omp parallel for schedule(static) private(j,x,y,temp,r) num_threads(TN)
	for(i=0;i<l+1;i++)
	for(j=0;j<m+1;j++)
	{
		x=(i-0.5)*dx;
...
//		NS();
//		CH();
        reinitiate_interface(iteration);
...
void flow_initialization(void)
{
	...
	#pragma omp parallel for schedule(static) private(j,x,y,dr) num_threads(TN)
    for(i=0;i<l;i++)
	for(j=0;j<m-1;j++)
    {
        x=i*dx-xc; y=(j+0.5)*dy-yc; dr=sqrt(x*x+y*y)+1.0E-8;
        u2[i][j]=x/dr; u3[i][j]=u2[i][j];
    }
	...
	for(j=0;j<m;j++)
    {
        x=(i+0.5)*dx-xc; y=j*dy-yc; dr=sqrt(x*x+y*y)+1.0E-8;
        v2[i][j]=y/dr; v3[i][j]=v2[i][j];
    }
}
...
void diffuse_initialization(void)
{
	...
	for(j=0;j<m+1;j++)
	{
		x=(i-0.5)*dx;
		y=(j-0.5)*dy;
		r=sqrt((x-xc)*(x-xc)+(y-yc)*(y-yc))+1.0E-9;
		temp=r-rad*(1.0+0.4*(x-xc)*(x-xc)/r/r);
//		temp=-1.0*temp;
...
void save_particle(void)
{
    ...
    fclose(fp);
    printf("output %s ,surf_sum=%lf, np=%d \n",part_name,surf_sum,np);
}
