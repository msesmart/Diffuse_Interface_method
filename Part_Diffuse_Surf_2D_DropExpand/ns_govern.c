double sigma[l+1][m+1]; /* nondimensional surface tension */
double surfx[l-1][m-1]={0.0},surfy[l-1][m-1]={0.0},st;
double preU[l][m-1],preV[l-1][m]={0},rhsu[l][m-1],rhsv[l-1][m],pot[l+1][m+1];

void surface_tension_geo(void)
{
	int i,j;
	double b1,b2,b3,b4,n_x,n_y,temp;
	...
	#pragma omp parallel for schedule(static) collapse(2) private(b1,b2,b3) num_threads(TN)
	for(i=1;i<l;i++)
	for(j=1;j<m;j++)
	{
	...
	#pragma omp parallel for schedule(static) collapse(2) private(b1,b2,b3,b4,n_x,n_y,temp) num_threads(TN)
	for(i=0;i<l-1;i++)
	for(j=0;j<m-1;j++)
	{
	    temp=0.5*(fi[i][j+1]+fi[i+1][j+1]);
	    if(temp*temp*(1.0-temp)*(1.0-temp)>0.0000158)
        {
